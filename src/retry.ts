...
export const RETRY_STRATEGIES = {
  EXP: (options: {
    ...
    return async (args: RetryArgs) => {
      isNumber(retryDelay);
      const delayMs = isNumber(retryDelay)
        ? retryDelay
        : retryDelay(args.count);
      const shouldRetry = () => {
        if (retryCount < args.count)
          return {
            ...args,
            retry: false,
          };
        if (retryOnError(args.err))
          return {
            ...args,
            retry: true,
          };
...
export type RetryFn = (
  args: RetryArgs
) => Promise<RetryArgs & { retry: boolean }>;

const defaultRetryStrategy = RETRY_STRATEGIES.EXP({
  retryCount: 5,
  retryDelay: count => Math.pow(count, 2) * 1000,
  retryOnError: () => true,
});

/**
 * retryOnFail function args
 * @param retryStrategy - Retry strategy function
 ...
 * @signature
 *    const newFn = P.retryOnFail(fn)
 * @example
 *    const request = (url: string) => axios.get(url)
 *    const requestWithRetry = P.retryOnFail(request)
 * @category Utility, Pipe
 */
export function retryOnFail<I extends any[], R>(
  fn: (...args: I) => Promise<R>,
  options?: RetryOnFailArgs
): (...args: I) => Promise<R>;
export function retryOnFail<I extends any[], R>(
  options?: RetryOnFailArgs
...
function _retryOnFail<I extends any[], R>(
  fn: (...args: I) => Promise<R>,
  options?: RetryOnFailArgs
): (...args: I) => Promise<R> {
  ...
  return (...args: I) => {
    ...
    return fn(...args).catch(async e => {
      ...
      while (true) {
        const retryFDS: RetryArgs & { retry: boolean } = await retryFn(
          prev || { count: 0, err, startTime, logger: options?.logger }
        );
        if (retryFDS.retry) {
